...
def _build_prompt(
    ...
    system = (
        ...
        "核となる価値です。\n"
        "- 「該当する事例が見つかりません」「分かりません」と答えるのは、出典コンテキストの"
        "内容が質問と本当に無関係な場合に限ってください。\n"
        "- 質問文自体が部署横断のプロジェクト知見・教訓についての検索質問になっていない場合"
        "（雑談・挨拶・アシスタント自身への感想など）は、出典コンテキストに何が含まれていても"
        "それに無理にこじつけず、「該当する事例が見つかりません」と答えてください。\n"
