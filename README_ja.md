...
>
> - ファイル形式（Markdown/Word/Excel/PowerPoint/PDF）を部署に固定せず、レポートごとにランダム割当にする設計。部署間の非統一性は用語（見出しの言い回し）側で表現する
> - 各レポートに結果画像を1枚埋め込み、ingest時にVLMでキャプション化して検索対象に含める設計
> - 全処理をローカルLLMで完結させ、外部送信ゼロにするアーキテクチャ選択（機密設計情報を扱う製造業を想定した現実的な制約）
> - DAG単位でのノード分解と、依存のないノード（retrieval/generation）の並列実装、各ノード完了ごとに別ベンダーAI（codex CLI）の独立レビューを必須ゲートにする、という開発プロセスの設計
