...
「ノード」とは、入力と出力の境界がはっきりした1つの役割のまとまりのこと。このプロジェクトでは、
たまたまノードA〜Fがそれぞれ1つのファイル（モジュール）に対応している（6つの役割それぞれが
1ファイルにまとめるのにちょうどいい大きさだったため。一般には、ノードの粒度は設計次第で、
1つの関数がノードになることも、複数ファイルからなるサブシステムが1ノードになることもある）。

各ノードは内部の処理を外に漏らさない。例えば`retrieval.py`には`_bm25_search`・`_vector_search`
のような非公開（`_`始まり）の内部関数が複数あるが、外部に公開しているのは`search()`という
1つの関数だけ。BM25なのかベクトル検索なのか、どうリランキングしているかは、呼び出す側
（`app.py`や`eval.py`）は一切知らなくていい。`generation.py`も同様に`answer_question()`だけを
公開し、プロンプトの組み立て方などは内部に閉じている。

### ノード間の接続方法

矢印1本ごとに、実際の接続方法は異なる。

- **A→B（datagen→ingest）**: ファイル経由。Aはレポートファイルをディスクの`data/synth_reports/`に書き出し、Bはそのファイルを読むだけ。Pythonのimportによる結合はゼロ（`ingest.py`は`datagen.py`を一切importしていない）。
- **B→C・D（ingest→retrieval/generation）**: ChromaDB（ディスク上の永続DB）経由でデータが渡る。C・DがimportしているのはBの処理関数ではなく、`Chunk`という**データ型の定義だけ**（`from .ingest import Chunk`）。Bがどう処理したかは知らず、結果の「形」だけ知っている。
- **C・D→E・F（retrieval/generation→eval/app）**: ここだけ通常のPython関数呼び出し。`eval.py`・`app.py`はそれぞれ`search()`・`answer_question()`を直接importして呼び出す。

依存が強い箇所ほどコードの結合も強く（3番目は直接関数呼び出し）、弱い箇所ほど結合も弱い（1番目はファイルのみ、2番目は型のみ）という対応になっている。

### 独立なノードはどこか

6ノード・6本の矢印からなるこのDAGで、「矢印で直接結ばれていない」ノードの組み合わせはC・D（retrieval・generation）の1組だけ。両方ともBには依存するが、互いには依存しない。

| ペア | 依存関係 | 並列実装できたか |
| --- | --- | --- |
| A-B, B-C, B-D, C-E, D-E, E-F | あり | できない（依存先の完成を待つ） |
| **C-D** | **なし** | **できた（実際に2つのAgentへ並列実装）** |

ここで注意が要るのは、「DAGが非巡回（ループがない）であること」と「特定のノード同士が独立であること」は別の話、という点。非巡回は「矛盾なく構築順序を決められるか」というグラフ全体の健全性の条件で、これが崩れるとそもそも構築が成立しない（AがBに依存し、BもAに依存していたら、どちらを先に作ればいいか決まらない）。一方、独立性は「このノードとこのノードの間に矢印（経路）があるか」というペアごとの話で、非巡回なグラフでも一直線の依存（A→B→C→D→E→F）なら並列化の余地はゼロになる。今回たまたまC・Dの間にだけ矢印がなかった、という**グラフの形そのもの**が、並列実装という利点を生んでいる。
...
念のため補足すると、「処理の順番を決められること」自体はグラフエンジニアリング特有の利点ではない。複雑なシステムを作る以上、AIでも人間でも、依存関係を整理して順番を決める作業はどのみち必要になる。このプロジェクトの文脈で実際に効いているのは、依存関係を明示的なグラフとして書き出した（ノード化した）からこそ見えるようになった2つの効果——「順番をつけなくていい部分（C・D）が発見でき、並列化できたこと」と「ノード単位で区切って狭い範囲でレビュー・検証できたこと」——で、これはAI協調開発（複数Agentが並行して作業し、別ベンダーAIが検証する体制）だからこそ特に価値が大きい。
